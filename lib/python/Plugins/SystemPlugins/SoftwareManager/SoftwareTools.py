...
#10.10.2014
# remove picons from list as destination is not handled, they end up in root.
        ...
        if result:
            if self.list_updating:
                ...
                for x in result.splitlines():
                    tokens = x.split(' - ')
                    name = tokens[0].strip()
                    if (not any(name.endswith(x) for x in self.unwanted_extensions)) and ('-picons' not in name):
                        l = len(tokens)
