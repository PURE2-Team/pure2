...
config.plugins.configurationbackup.backupdirs = ConfigLocations(default=[eEnv.resolve('${sysconfdir}/enigma2/'), '/etc/CCcam.cfg', '/usr/keys/', '/etc/network/interfaces', '/etc/wpa_supplicant.conf', '/etc/opkg/pure2-extra-feed.conf', '/etc/wpa_supplicant.ath0.conf', '/etc/wpa_supplicant.wlan0.conf', '/etc/resolv.conf', '/etc/default_gw', '/etc/hostname', eEnv.resolve("${datadir}/enigma2/keymap.usr")])
...
def checkConfigBackup():
    ...
    if boxtype in ('maram9', 'classm', 'axodin', 'axodinc', 'starsatlx', 'genius', 'evo', 'galaxym6'):
        parts.append(('Internal Backup area','/media/backup'))
    for x in parts:
        if x[1] == '/':
            parts.remove(x)
    if len(parts):
...
class ImageWizard(WizardLanguage, Rc):
    ...
    def listDevices(self):
        list = [ (r.tabbedDescription(), r.mountpoint) for r in harddiskmanager.getMountedPartitions(onlyhotplug = False) ]
