...
config.plugins.configurationbackup.backupdirs = ConfigLocations(default=[eEnv.resolve('${sysconfdir}/enigma2/'), '/etc/CCcam.cfg', '/usr/keys/', '/etc/opkg/external-extra-feed.conf',
																		 ...
																		 '/etc/volume.xml', '/etc/enigma2/ci_auth_slot_0.bin', '/etc/enigma2/ci_auth_slot_1.bin',
																		 '/etc/davfs2/', '/etc/tuxbox/config/', '/etc/auto.network', '/etc/enigma2/automounts.xml', '/etc/rc.local',
...
class BackupScreen(Screen, ConfigListScreen):
	...
	def doBackup(self):
		...
		try:
			...
			cmd4 = "tar -czvf " + self.fullbackupfilename + " " + self.backupdirs
