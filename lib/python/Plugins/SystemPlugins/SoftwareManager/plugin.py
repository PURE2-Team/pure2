...
if boxtype in ('maram9', 'classm', 'axodin', 'axodinc', 'starsatlx', 'genius', 'evo', 'galaxym6') and not os.path.exists("/media/hdd/backup_%s" %boxtype):
...
config.plugins.configurationbackup.backupdirs = ConfigLocations(default=[eEnv.resolve('${sysconfdir}/enigma2/'), '/etc/CCcam.cfg', '/usr/keys/', '/usr/bin/cam/', '/etc/auto.network', '/etc/enigma2/automounts.xml',
																		 '/etc/tuxbox/config/', '/usr/script/', '/var/tuxbox/config/',
																		 ...
																		 '/etc/wpa_supplicant.wlan0.conf', '/etc/resolv.conf', '/etc/default_gw', '/etc/hostname', '/etc/opkg/pure2-extra-feed.conf',
...
#                    parts = [ (r.description, r.mountpoint, self.session) for r in harddiskmanager.getMountedPartitions(onlyhotplug = False)]
                    for x in parts:
                        if not os.access(x[1], os.F_OK|os.R_OK|os.W_OK) or x[1] == '/':
                            parts.remove(x)
                    if len(parts):
                        self.session.openWithCallback(self.backuplocation_choosen, ChoiceBox, title = _("Please select medium for backup"), list = parts)
...
#### TODO! Bad, bad stuff. /usr/share/meta has different names than packages!!!
	...
	#print 'CURRENT ', current
		...
		#print 'CURRENT[7] ', current[7]
                if current[7] is not '':
                    detailsfile = iSoftwareTools.directory[0] + "/" + current[0]  # was [1] = 'enigma2-plugins', so 0, 2 or 3 !!!
		    #print 'CURRENT[1] ', current[1]
		    #print 'DETAILSFILE ', detailsfile	
...
###### not enough info or bad call /usr/share/meta is there but not used
                        text = current[1]
                        self.session.open(MessageBox, "%s" % text, MessageBox.TYPE_INFO, timeout = 0)
            elif self.currList == "category":
                self.prepareInstall()
                if len(self.cmdList):
                    self.session.openWithCallback(self.runExecute, PluginManagerInfo, self.skin_path, self.cmdList)
    ...
    def buildPacketList(self, categorytag = None):
        ...
        self.packetlist = []
        for package in iSoftwareTools.available_packetlist:
            if "enigma2-plugin" in package[0] or "enigma2-skin" in package[0]:
                if "meta" not in package[0]:
                    if "enigma2-skins" not in package[0]:
                        if "enigma2-plugins" not in package[0]:
                            if "enigma2-plugin-pli" not in package[0]:
                                if "-po" not in package[0][-3]:
                                    name = package[0]
                                    packagename = package[0]
                                    details = package[2]
                                    if "enigma2-plugin-skin" in package[0]:
                                        foundtag = "skins"
                                    elif "enigma2-skin" in package[0]:
                                        foundtag = "skins"
                                    elif "enigma2-plugin-extensions" in package[0]:
                                        foundtag = "extensions"
                                    elif "enigma2-plugin-private" in package[0]:
                                        foundtag = "extensions"
                                    elif "enigma2-plugin-camd" in package[0]:
                                        foundtag = "camd"
                                    elif "enigma2-plugin-system" in package[0]:
                                        foundtag = "systemplugins"
                                    elif "enigma2-plugin-security" in package[0]:
                                        foundtag = "systemplugins"
                                    elif "enigma2-plugin-drivers-dvb" in package[0]:
                                        foundtag = "drivers"
                                    elif "enigma2-plugin-settings" in package[0]:
                                        foundtag = "satsettings"
                                    elif "bi58-stb-team-settings" in package[0]:
                                        foundtag = "satsettings"
                                    elif "enigma2-plugin-picons" in package[0]:
                                        pass
#                                        foundtag = "picons"
                                    else:
                                        foundtag = "other"
                                    if categorytag == foundtag:
                                        self.packetlist.append([name, details, packagename])
...
###### changed details/description
                self.list.append(self.buildEntryComponent(name, _(description), _(details), packagename, status, selected = selectState))
            else:
                if selectState == True:
                    status = "install"
                else:
...
#                categorytag = "picons"
            ...
            if categorytag not in self.categories:
                self.categories.append(categorytag)
                self.categoryList.append(self.buildCategoryComponent(categorytag))
    ...
    def buildCategoryComponent(self, tag = None):
        ...
        if tag is not None:
            if tag == 'extensions':
                return(( _("Extensions"), _("View list of available extensions" ), tag, divpng ))
            elif tag == 'skins':
                return(( _("Skins"), _("View list of available skins" ), tag, divpng ))
            elif tag == 'systemplugins':
                return(( _("Systemplugins"), _("View list of available system extensions" ), tag, divpng ))
            elif tag == 'camd':
                return(( _("Camd"), _("View list of available Cams" ), tag, divpng ))
            elif tag == 'drivers':
                return(( _("DVB-drivers"), _("View list of available DVB-drivers (usb-dvb)" ), tag, divpng ))
            elif tag == 'satsettings':
                return(( _("Settings"), _("View list of available sat-settings" ), tag, divpng ))
            elif tag == 'picons':
                return(( _("Picons"), _("View list of available picons" ), tag, divpng ))
            else: # dynamically generate non existent tags
                return(( str(tag), _("View list of ") + str(tag) + _(" packages." ), tag, divpng ))
...
def Plugins(path, **kwargs):
    ...
    if not config.plugins.softwaremanager.onSetupMenu.value and not config.plugins.softwaremanager.onBlueButton.value:
        list.append(PluginDescriptor(name=_("Software management"), description=_("Manage your receiver's software"), where = PluginDescriptor.WHERE_PLUGINMENU, needsRestart = False, icon='plugin.png', fnc=UpgradeMain))
