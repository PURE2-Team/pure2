...
def checkMenu():
	...
	menustyle = config.estuary.MenuStyle.value
	if menustyle != 'Standard':
		return False
	else:
		return True
...
def main(session, *args, **kwargs):
	if checkMenu():
		...
		session.open(MessageBox, _("Menu-Style is not Standard. Please change it to Standard!"), type = MessageBox.TYPE_INFO,timeout = 20 )
