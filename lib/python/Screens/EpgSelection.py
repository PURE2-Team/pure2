...
class EPGSelection(Screen, HelpableScreen):
	...
	def openIMDb(self):
		#FIRST TRY WITH NEW IMDb PLUGIN. IF ERROR TRY WITH OLD
		try:
			#from Plugins.Extensions.IMDb.plugin import IMDB, IMDBEPGSelection
			from Plugins.Extensions.IMDbFNC.plugin import FNCimdbAutoStart
			try:
				cur = self['list'].getCurrent()
				event = cur[0]
				...
				name = ''
			self.session.open(FNCimdbAutoStart, name)
		except ImportError:
