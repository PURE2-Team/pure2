...
class PowerLost(Screen):
	...
	def showMessageBox(self):
		if config.usage.boot_action.value == 'normal':
			...
			timer = config.usage.shutdownNOK_action_time.value
			self.session.openWithCallback(self.MsgBoxClosed, MessageBox, message, MessageBox.TYPE_YESNO, timeout = int(timer), default = True)
		else:
