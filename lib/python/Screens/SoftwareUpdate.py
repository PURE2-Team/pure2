...
class UpdatePlugin(Screen):
				...
				if self.total_packages:
					...
					if self.total_packages > 150:
						...
						choices = [(_("Update and reboot (recommended)"), "cold"),
							...
							(_("Show packages to be updated"), "listofpackages"),
							(_("Cancel"), "")]
	...
	def startActualUpgrade(self, answer):
		...
		elif answer[1] == "listofpackages":
			self.text = "\n echo ".join([x[0] for x in sorted(self.ipkg.getFetchedList(), key=lambda d: d[0])])
			from Screens.Console import Console
			self.session.openWithCallback(boundFunction(self.ipkgCallback, IpkgComponent.EVENT_DONE, None), Console, title = _("Packages to update"), cmdlist = ['echo " \n"' + self.text])
		else:
	...
	def exit(self):
		if not self.ipkg.isRunning():
			if self.packages != 0 and self.error == 0 and self.channellist_only == 0:
				self.session.openWithCallback(self.exitAnswer, MessageBox, _("Update completed. Do you want to reboot your %s %s?") % (getMachineBrand(), getMachineName()))
			else:
				self.close()
		else:
