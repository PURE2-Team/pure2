...
from boxbranding import getBoxType, getMachineProcModel, getMachineBuild, getMachineBrand, getMachineName, getBrandOEM
	...
	def bufferChanged(self):
		if self.mayShow:
			...
			if info:
				...
				if value and value != 100 and config.usage.show_buffering.value:
...
class InfoBarChannelSelection:
	...
	def LeftPressed(self):
		if config.plisettings.InfoBarEpg_mode.value == "3" and config.usage.show_second_infobar.value != "INFOBAREPG":
			self.openInfoBarEPG()
		else:
			self.zapUp()
	...
	def doPause(self, pause):
		if pause:
			...
			if eDVBVolumecontrol.getInstance().isMuted():
				DVBVolumecontrol.getInstance().volumeUnMute()
...
class InfoBarExtensions:
	...
	def __init__(self):
		...
		#self.addExtension(extension = self.getNetworkRestart, type = InfoBarExtensions.EXTENSION_LIST) #Network restart out for now
	...
	def getNetworkRestartname(self):
		return _("Restart Network")

	#Network restart out for now
	
	# def getNetworkRestart(self):
		# if config.network.shownetworkrestart.value:
			# return [((boundFunction(self.getNetworkRestartname), boundFunction(self.openNetworkRestart), lambda: True), None)]
		# else:
			# return []
...
#	def openNetworkRestart(self):
#		from Plugins.Extensions.PowerboardCenter.PBnetinfo import PBRestartNetwork
...
class InfoBarHdmi:
	...
	def SleepTimer(self):
			...
			self.Console.ePopen('/usr/bin/clikey KEY_GREEN KEY_RIGHT KEY_RIGHT')
	...
	def CAMrestart(self):
			from Plugins.Extensions.DreamCC.plugin import DreamCCAuto
			DreamCCAuto()
			self.session.open(MessageBox,"  Active CAM has just been restarted.", MessageBox.TYPE_INFO, timeout=5, close_on_any_key=True) #fugly, but needed to pop keystroke
	...
	def MP2(self):
			try: # falls es nicht installiert ist
				...
				self.session.open(MessageBox,"  Media Player 2 is NOT installed.", MessageBox.TYPE_INFO, timeout=5, close_on_any_key=True) #fugly, but needed to pop keystroke
