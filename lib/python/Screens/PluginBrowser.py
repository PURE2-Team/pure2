...
config.pluginfilter.hdscsettings = ConfigYesNo(default = True)
...
config.pluginfilter.camd = ConfigYesNo(default = True)
config.pluginfilter.systemplugins = ConfigYesNo(default = True)
config.pluginfilter.external = ConfigYesNo(default = True)
...
class PluginBrowser(Screen, ProtectedScreen):
	def __init__(self, session):
		...
		self["key_blue"] = Label(_("Hold plugins"))
	...
	def CheckPure2Fconfirm(self, confirmed):
		if confirmed:
			self.download2() # continue as usual and waste a lot time :)
		else:
			self.PluginDownloadBrowserClosed() # return to pluginbrowser 

# commadns arre alreday loaded in E2, subprocess and urllib2 are not, try to make it simple
# returns 0 if OK, anything else various errors
	def CheckPure2feeds(self):
		#import commands
		from Screens.MessageBox import MessageBox
		
		try:
			#web = urllib.urlopen('http://pur-e2.club/OU/newfeed/extra-mips32/Packages.gz')
			web = urllib.urlopen('http://pur-e2.club/OU/feed_check.txt')
			if web.code == 200: #(200 - datoteka postoji, 404 - datoteka ne postoji)
				self.download2()
			else :
				confq = self.session.openWithCallback(self.CheckPure2Fconfirm, MessageBox, text = _("Main PurE2 feeds are not available.\nAdditional checking may take few minutes!\n\nDo you wish to continue anyway?"), type = MessageBox.TYPE_YESNO, default = False)
				confq.setTitle(_("PurE2 feeds problem:"))
		except IOError:
			confq = self.session.openWithCallback(self.CheckPure2Fconfirm, MessageBox, text = _("Main PurE2 feeds are not available.\nAdditional checking may take few minutes!\n\nDo you wish to continue anyway?"), type = MessageBox.TYPE_YESNO, default = False)
			confq.setTitle(_("PurE2 feeds problem:"))
				
		#(err,out) = commands.getstatusoutput('wget --spider  -T 10  -P  /tmp http://pur-e2.club/OU/newfeed/extra-mips32/Packages.gz')
		#if err:
			# confq = self.session.openWithCallback(self.CheckPure2Fconfirm, MessageBox, text = _("Main PurE2 feeds are not available.\nAdditional checking may take few minutes!\n\nDo you wish to continue anyway?"), type = MessageBox.TYPE_YESNO, default = False)
			# confq.setTitle(_("PurE2 feeds problem:"))
		# else :
...
class PluginDownloadBrowser(Screen):
	...
	def createPluginFilter(self):
		...
		if config.pluginfilter.hdscsettings.value:
			self.PLUGIN_PREFIX2.append('enigma2-plugin-hdsc-settings-')
		...
		if config.pluginfilter.camd.value:
			self.PLUGIN_PREFIX2.append(self.PLUGIN_PREFIX + 'camd')
		if config.pluginfilter.systemplugins.value:
			self.PLUGIN_PREFIX2.append(self.PLUGIN_PREFIX + 'systemplugins')
		if config.pluginfilter.external.value:
			self.PLUGIN_PREFIX2.append(self.PLUGIN_PREFIX + 'external')
	...
	def dataAvail(self, str):
		...
		for x in lines:
			...
			if len(plugin) >= 1:
				if not plugin[0].endswith('-dev') and not plugin[0].endswith('-staticdev') and not plugin[0].endswith('-dbg') and not plugin[0].endswith('-doc') and not plugin[0].endswith('-src') and not plugin[0].endswith('-meta') and not plugin[0].endswith('-po'):
					# Plugin filter
...
class PluginFilter(ConfigListScreen, Screen):
	...
	def createSetup(self):
		...
		self.list.append(getConfigListEntry(_("HDSC-Settings"), config.pluginfilter.hdscsettings, _("This allows you to show HDSC-Settings in downloads")))
		...
		self.list.append(getConfigListEntry(_("Camd"), config.pluginfilter.camd, _("This allows you to show softcams modules in downloads")))
